...
OMXTerm is a **browser SSH terminal**: you open a web page, type into it, and the
keystrokes end up in a real shell on a server you chose. The hard part is not
drawing a terminal — it is doing that _without_ turning a public URL into an open
SSH proxy.
...
The boundary OMXTerm promises is **safe brokering**. It does _not_ decide whether
your remote user is root — that is the SSH target's job (see
...
1. **Rate-limits by client IP first.** `InMemoryAccessRateLimiter` allows 10
   ...
   a timing-safe compare blocks timing leaks, not guessing. Over the limit → HTTP
   429 with `Retry-After`.
2. **Compares the token in constant time.** `safeEqualText` uses
   `timingSafeEqual`, so a wrong token takes the same time whether the first byte
   or the last byte is wrong. Wrong token → records a failure and returns 401.
3. **On success**, resets the limiter and mints two secrets:
   ...
   - a **device token** (`InMemoryDeviceTokenStore`) — a second random per-browser
     secret bound to that session, also stored hashed, 12-hour TTL.
...
> **Why two tokens?** The session says "this browser passed the gate." The device
> token is what later binds ticket issuance and the WebSocket upgrade to the _same_
> browser. Only their **hashes** live on the server, so a leaked store dump cannot
> be replayed as a cookie.

`GET /api/me` is a cheap read that just reports whether the current cookies still
validate — the UI uses it to skip the gate on reload.
...
`hostVerifier`, computes a `SHA256:<base64>` fingerprint, and **returns `false` so
it never authenticates** — it just reads the key and hangs up. The fingerprint
comes back to the browser, which shows it to the user.

The user eyeballs it (against what they know the server's key to be) and accepts.
...
> A ticket is a deliberate, one-shot capability: short-lived, used exactly once,
> and tied to the session/device/Origin that requested it.
...
- **The fingerprint is re-verified for real.** `hostVerifier` recomputes the
  ...
  enforcement.
- **A remote PTY is allocated** via `shell` (`xterm-256color`, initial 120×34), so
  full-screen tools like `vim`, `htop`, and `tmux` behave.
...
Output is UTF-8 decoded per stream so a multi-byte character split across two PTY
chunks is reassembled instead of rendering as `�`
...
Audit is **metadata only** — security-relevant lifecycle, never terminal content.
...
- It does **not** keep a persistent `known_hosts` — host-key trust is per session.
- It does **not** save private keys, passphrases, connection profiles, or terminal
  transcripts.
...
[`docs/prd-mvp-web-ssh-terminal.md`](./prd-mvp-web-ssh-terminal.md). A production
version would add persistent host-key pinning, a durable shared store, a stronger
account model, and mature backpressure/lifecycle handling.
